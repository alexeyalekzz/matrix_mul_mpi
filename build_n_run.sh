...
if [ "$#" -lt 6 ]; then
    ...
    mpicxx $OPT main_row.cpp -o mmul_row && { mpiexec --use-hwthread-cpus -n "$2" mmul_row "${@:3}"; }
elif [[ "$1" == "col" ]]; then
    if [ $(("$4" % "$2")) -ne 0 ]; then
    ...
    mpicxx $OPT main_col.cpp -o mmul_col && { mpiexec --use-hwthread-cpus -n "$2" mmul_col "${@:3}"; }
elif [[ "$1" == "block" ]]; then

    mpicxx $OPT main_blk.cpp -o mmul_blk && { mpiexec --use-hwthread-cpus -n "$2" mmul_blk "${@:3}"; }
fi
